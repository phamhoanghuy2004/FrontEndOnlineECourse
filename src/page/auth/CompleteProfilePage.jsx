...
// ===== Helpers (Tái sử dụng để chuyển hướng thông minh) =====
...
const CompleteProfilePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isValid) {
            ...
            setGlobalError("Vui lòng hoàn thiện thông tin bị thiếu.");
            return;
        }
        ...
        try {
            await userApi.completeProfileApi(profileData);

            // 1. Cập nhật dữ liệu ngay lập tức vào LocalStorage & Context
            ...
            // 💥 2. CHUYỂN HƯỚNG THÔNG MINH THEO ROLE (Tránh việc ai cũng bị ném về '/')
            ...
            // 💥 FIX: Đảm bảo error luôn lấy chuỗi message từ Axios Interceptor
            setGlobalError(error.message || "Có lỗi xảy ra khi cập nhật hồ sơ");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="w-full max-w-[450px] px-4 relative z-10">
                ...
                >
                    <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-emerald-900/10 border border-white/50 p-6 md:p-8">
                        ...
                        {/* Chừa sẵn không gian tĩnh (h-6) */}
                        <div className="h-6 mt-1 mb-2 flex justify-center items-center">
                            {globalError && (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-1">
                            <div className="relative pb-[14px]">
                                <InputField
                                    label="Địa chỉ" icon={FaMapMarkerAlt} type="text" required={true} size="compact"
                                    placeholder="Nhập địa chỉ của bạn"
                                    value={profileData.address} error={!!errors.address}
                            ...
                            <div className="relative pb-[14px]">
                                <InputField
                                    label="Ngày sinh" icon={FaCalendarAlt} type="date" required={true} size="compact"
                                    value={profileData.dob} error={!!errors.dob}
                            ...
                            <div className="relative pb-[14px]">
                                <InputField
                                    label="Nghề nghiệp" icon={FaBriefcase} type="text" required={true} size="compact"
                                    placeholder="Học sinh, Sinh viên, Người đi làm..."
                                    value={profileData.jobTitle} error={!!errors.jobTitle}
                            ...
                            >
                                <span className="relative z-10">{isLoading ? "Đang cập nhật..." : "Lưu Thông Tin"}</span>
